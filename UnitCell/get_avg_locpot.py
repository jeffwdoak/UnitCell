...
"""
Script to extract the electrostatic potential averaged around each atom from an
OUTCAR file associated with a unitcell.
...
for i in range(poscar.num_atoms):
    print (i+1, poscar.atom_names[i], poscar.atom_positions[i, 0],
           poscar.atom_positions[i, 1], poscar.atom_positions[i, 2],
           avg_loc_pots[i])
sys.exit()
